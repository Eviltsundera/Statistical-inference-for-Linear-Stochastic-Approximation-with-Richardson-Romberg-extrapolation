...
terminal-iterate setting.

We consider the recursion
$theta_k = theta_(k-1) - alpha_k (A(Z_k) theta_(k-1) - b(Z_k))$ with
$alpha_k = alpha$ constant.
...
linearized term
$J_k^((0, alpha)) = -alpha sum_(l=1)^k B_alpha^(k-l) epsilon.alt(Z_l)$,
with $J_0^((0, alpha)) = 0$.
...
A standard PR-averaged decomposition (cf. Chapter 4 for the full derivation)
yields $sqrt(n) (overline(theta)_n^((alpha)) - theta^*) = W + D_1$, where
the leading martingale-like term is
$
W = -frac(1, sqrt(n)) sum_(l=1)^(n-1) Q_l epsilon.alt(Z_l),
quad
...
D_1
  = frac(1, sqrt(n)) sum_(k=0)^(n-1) B_alpha^k (theta_0 - theta^*)
    + frac(1, sqrt(n)) sum_(k=1)^(n-1) R_k^((alpha)).
$
