...
The depth-zero martingale in @lem:burn-poisson-decomp is normalized at the
effective sample size $m = n - n_0$.
// We assume $m >= n slash 2$ and impose the mixing-scale burn-in lower bounds
// with logarithmic factors, so the inhomogeneous concentration term obtained
// from $sqrt(p thin t_"mix" thin sum c_i^2)$ and then written as
...
// with $C_("burn,Q")$ replacing the full-window weight bound.
Set
$
...
#theorem[
  *(Burned-in martingale Berry--Esseen bound.)*
  ...
  universal constants in @lem:external-martingale-be, such that for every
  $n >= 3$,
  $
  d_K lr((
...
$log^(3 slash 4)(n) m^(-1 slash 4)$ bound. The bounded-increment terms give
the second displayed term. $square$
